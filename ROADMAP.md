...
**Not yet done:**
- `labels-for-analysis` (the branch all prior harvested data — 6-22/6-29-fullrun, cluster_results.csv — was generated with) is untouched on purpose, to keep that data reproducible. `lazy-adjacency-relabelled` is a candidate, not yet the reference. *(2026-08-28: the NDS backport added one 2-line commit on top of this branch in the laptop clone, `2180663` → `861a84f`. Reproducibility is unaffected — the pinned hash `2180663` still resolves to the same commit and `cluster_dist.sh` builds by hash, not by branch name — but the branch head is no longer `2180663`. Unpushed; `ciaranm/…` and the cluster clone are untouched.)*
