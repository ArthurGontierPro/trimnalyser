...
println()
println(rpad("family", 9), rpad("tool", 14), rpad("n", 6), rpad("sgm(compr)", 12),
        rpad("med(compr)", 12), rpad("sgm(trim s)", 13), rpad("med(chk s)", 12), "failed")
println("-"^84)
summary = Dict{Tuple{String,String},Any}()
for fam in [FAMS; "ALL"], tool in present
    sel = fam == "ALL" ? rows : filter(r -> famof(r) == fam, rows)
    isempty(sel) && continue
    compr = Float64[]; tt = Float64[]; ck = Float64[]; nfail = 0
    for r in sel
    ...
    summary[(fam, tool)] = (n = length(compr), sgm = sgm(compr), med = med(compr),
                            tsgm = sgm(tt), tmed = med(tt), cmed = med(ck), fail = nfail)
    s = summary[(fam, tool)]
    println(rpad(fam, 9), rpad(tool, 14), rpad(s.n, 6),
            rpad(round(s.sgm; digits = 2), 12), rpad(round(s.med; digits = 2), 12),
...
for other in filter(!=("ta"), present)
    ...
    for fam in [FAMS; "ALL"; "companionLV"]
        ...
        sz, ti = headtohead(sel, other)
        isempty(sz) && continue
        println(rpad(fam, 13), "n=", rpad(length(sz), 6),
                "  smaller by sgm ", round(sgm(sz); digits = 2),
                " (median ", round(med(sz); digits = 2), ")",
    ...
    szA, tiA = headtohead(rows, other)
    szL, tiL = headtohead(filter(r -> r["instance"] in
                     ["LVg19g36","LVg2g3","LVg34g51","LVg5g24","LVg7g25"], rows), other)
    println()
    println("  sec:compress placeholders (vs $other):")
    println("    \\ph{X}   = ", round(sgm(szA); digits = 2), "   (n=", length(szA), ")")
    println("    \\ph{Y}   = ", round(sgm(tiA); digits = 2), "   (n=", length(tiA), ")")
    println("    \\ph{XLV} = ", round(sgm(szL); digits = 2), "   (n=", length(szL), ")")
...
function tex(io)
    ...
    println(io, "\\toprule")
    println(io, "Family & Trimmer & \$n\$ & failed & \\multicolumn{2}{c}{Compression} & Trim (s) & Check (s) \\\\")
    println(io, "& & & & sgm & median & sgm & median \\\\")
    println(io, "\\midrule")
    for fam in [FAMS; "ALL"]
        first = true
        for tool in present
            ...
            println(io, (first ? (fam == "ALL" ? "\\midrule\nAll" : fam) : ""), " & ",
                    ...
                    f2(s.sgm), " & ", f2(s.med), " & ", f2(s.tsgm), " & ", f2(s.cmed), " \\\\")
            first = false
        end
    end
